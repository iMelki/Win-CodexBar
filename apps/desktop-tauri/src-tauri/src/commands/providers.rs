...
pub struct DeepSeekPricingStatus {
    pub period: codexbar::providers::deepseek::pricing::PricingPeriod,
    pub current_local_time: String,
    pub next_transition_local_time: Option<String>,
    pub effective_local_time: String,
}

/// Build the DeepSeek pricing status payload for the given instant.
/// Pure: no notification mutation, no settings read.
pub(crate) fn build_deepseek_pricing_status(now: chrono::DateTime<Utc>) -> DeepSeekPricingStatus {
...
#[tauri::command]
pub fn get_deepseek_pricing_status() -> Option<DeepSeekPricingStatus> {
    if !Settings::load().enabled_providers.contains("deepseek") {
        return None;
    }
    Some(build_deepseek_pricing_status(Utc::now()))
}
